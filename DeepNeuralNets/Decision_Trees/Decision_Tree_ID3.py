...
# Kordel France
########################################################################################################################
# This file establishes the specification and driver for an ID3 decision tree model.
########################################################################################################################
...
"""
Class definition of the ID3 Decision Tree with all helper methods
"""
class Decision_Tree_ID3:
    ...
    def find_split(self, items, rule):
        """
        Finds the split point as a function of the entropies based on true and false values.
    ...
    def calculate_entropy(self, items):
        """
        Calculates the entropy at a specifc node given a specific decision path or subtree.
        :param items: list - the list of objects to calculate the entropy score over.
        :return entropy: float - a value indicating the entropy of the tree up to a specific point.
        """
        label_count: list = []
        all_items: list = []

        # build temporary lists to use for entropy construction, we don't want to alter the 'items' argument
        for index_i in range(0, len(ALL_LABELS)):
            label_count.append(float(ALL_LABELS[index_i]))
        for index_i in range(0, len(items)):
            all_items.append(items[index_i])

        # calculate the cumulative entropy through the summation of individual entropy
        entropy = sum([-count / len(all_items) * math.log(count / len(all_items), 2) if count else 0 for count in label_count])
        return entropy


    def calculate_information_gain(self, left_tree, right_tree, confidence):
        """
        ...
        info_gain: float = confidence - prob * self.calculate_entropy(left_tree) - (1 - prob) * self.calculate_entropy(right_tree)
        return info_gain
    ...
    def calculate_optimal_split(self, items):
        ...
        rule_optimal = None
        entropy = self.calculate_entropy(items)
        ...
        for feature in range(num_extracted_features):
            ...
            for value in values:
                ...
                gain_local = self.calculate_information_gain(items_true, items_false, entropy)
    ...
    def construct_decision_tree(self, items, threshold: float):
        """
        Takes the raw dataframe and constructs a ID3 decision tree by recursively definint rules until data runs out.
...
def reset_metrics():
    ...
    FINAL_NODES = []
    INFO_GAINS = []
    MIN_INFO_GAIN = math.inf
    TREE_STRING = ''
    ALL_LABELS = []


def train_ID3_decision_tree(training_data, training_labels, should_print: bool, threshold: float = math.inf):
    """
    This is the driver function for training the ID3 decision tree.
    ...
    :param threshold - if != 0.0, this value defines the stopping value for tree pruning.
    :return decision_tree_id3: Decision_Tree_ID3 - a fully constructed ID3 decision tree model.
    """
    decision_tree_id3 = Decision_Tree_ID3()
    decision_tree_id3.fit_data(training_data, training_labels, threshold)
    if should_print:
        decision_tree_id3.print_decision_tree_to_console()
    return decision_tree_id3


def calculate_metrics(testing_data, testing_labels, decision_tree):
    """
    Calculates the final accuracy achieved after the tree is constructed, trained and evaluated.
    ...
    :param decision_tree: Decision_Tree_ID3 - a fully constructed ID3 decision tree model.
    :param accuracy: float - the accuracy of the final model.
    """
...
if config.DEMO_MODE:
    ...
    def import_dataset(filepath: str, column: int):
        input_data: pd.DataFrame = pd.read_csv(filepath, header=None)[:100]
    ...
    def construct_decision_tree_and_evaluate(filename: str, column: int, threshold: float):
        ...
        decision_tree_id3 = train_ID3_decision_tree(training_data, training_labels, True, threshold)
        print(f'training time: ', str(datetime.now() - start_time))
        print('testing accuracy: ', float(calculate_metrics(testing_data, testing_labels, decision_tree_id3)))
        print(f'min info gain: {min(INFO_GAINS)}')
        print(f'length of decision tree: {len(FINAL_NODES)}')
        evaluate_decision_tree(testing_data, testing_labels, decision_tree_id3)


    if __name__ == '__main__':
        construct_decision_tree_and_evaluate('io_files/breast-cancer-wisconsin(1)_DATA.csv', 10, threshold=math.inf)
                                                                                                    # 1155660958
